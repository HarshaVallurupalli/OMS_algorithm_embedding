...
void StateProcessCommand::task()
{
	...
	switch (commandValue)
  {
		...
		case Command::GET_DISTANCE_AMPLITUDE:
			context.tofCos.setMovementDetectionSettings(false,4/*command.getMovementDetectionWindowSize()*/);
		...
		case Command::SET_ALGORITHM_SETTINGS:
			context.tofCos.setMovementDetectionSettings(true,4/*command.getMovementDetectionWindowSize()*/);
			break;
