  ...
  for (int y =0; y < dataHeader.height; y++)
   {
     for (int x = 0; x < dataHeader.width; x++)
     {
       if (selection.contains(x, y))
       {
           uint8_t valueMsb = static_cast<uint8_t>(pData[8*index+1+dataHeader.offset]);
           uint8_t valueLsb = static_cast<uint8_t>(pData[8*index+0+dataHeader.offset]);
           unsigned int value = static_cast<unsigned int>((valueMsb << 8) + valueLsb);
           statistics[0]->addSample(static_cast<double>(value));

           valueMsb = static_cast<uint8_t>(pData[8*index+3+dataHeader.offset]);
           valueLsb = static_cast<uint8_t>(pData[8*index+2+dataHeader.offset]);
           value = static_cast<unsigned int>((valueMsb << 8) + valueLsb);
           statistics[1]->addSample(static_cast<double>(value));

           valueMsb = static_cast<uint8_t>(pData[8*index+5+dataHeader.offset]);
           valueLsb = static_cast<uint8_t>(pData[8*index+4+dataHeader.offset]);
           value = static_cast<unsigned int>((valueMsb << 8) + valueLsb);
           statistics[2]->addSample(static_cast<double>(value));

           valueMsb = static_cast<uint8_t>(pData[8*index+7+dataHeader.offset]);
           valueLsb = static_cast<uint8_t>(pData[8*index+6+dataHeader.offset]);
           value = static_cast<unsigned int>((valueMsb << 8) + valueLsb);
           statistics[3]->addSample(static_cast<double>(value));
       }
