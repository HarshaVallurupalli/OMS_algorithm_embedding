...
void SurfacePlot::setColorFromVertexG(int ix, int iy, bool skip)
{
    ...
    RGBA col = (*datacolor_p)( actualDataG_->vertices[ix][iy][0], actualDataG_->vertices[ix][iy][1], actualDataG_->vertices[ix][iy][2]);
...
void SurfacePlot::createNormalsG()
{
    ...
    for(int i = 0; i <= actualDataG_->columns() - step; i += step)
    {
        for(int j = 0; j <= actualDataG_->rows() - step; j += step)
        {
            ...
            topv  = Triple(actualDataG_->vertices[i][j][0]+actualDataG_->normals[i][j][0], actualDataG_->vertices[i][j][1]+actualDataG_->normals[i][j][1], actualDataG_->vertices[i][j][2]+actualDataG_->normals[i][j][2]);
...
void SurfacePlot::readIn(GridData& gdata, double** data, unsigned int columns, unsigned int rows, double minx, double maxx, double miny, double maxy)
{
    ...
    ParallelEpiped hull = ParallelEpiped( Triple(gdata.vertices[0][0][0], gdata.vertices[0][0][1], tmin ),
        Triple( gdata.vertices[gdata.columns()-1][gdata.rows()-1][0], gdata.vertices[gdata.columns()-1][gdata.rows()-1][1], tmax ));
...
void SurfacePlot::Isolines2FloorG()
{
    ...
    for (int k = 0; k != isolines(); ++k)
    {
        ...
        for (int i = 0; i < cols-step; i += step)
        {
            ...
            {
                t[0] =  Triple(	actualDataG_->vertices[i][j][0], actualDataG_->vertices[i][j][1], actualDataG_->vertices[i][j][2]);

                col = (*datacolor_p)(t[0].x,t[0].y,t[0].z);
                glColor4d(col.r, col.g, col.b, col.a);

                t[1] =  Triple(	actualDataG_->vertices[i+step][j][0], actualDataG_->vertices[i+step][j][1], actualDataG_->vertices[i+step][j][2]);
                t[2] =  Triple(	actualDataG_->vertices[i+step][j+step][0], actualDataG_->vertices[i+step][j+step][1], actualDataG_->vertices[i+step][j+step][2]);
                t[3] =  Triple(	actualDataG_->vertices[i][j+step][0], actualDataG_->vertices[i][j+step][1], actualDataG_->vertices[i][j+step][2]);
                ...
                for(int m = 0; m!=4; ++m)
                {
                    ...
                    if ((val>=t[m].z && val<=t[mm].z) || (val>=t[mm].z && val<=t[m].z))
                    {
                        ...
                        {
                            intersection.emplace_back(t[m]);
                            intersection.emplace_back(t[mm]);
                            continue;
                        }

                        lambda =  (val - t[m].z) / diff;
                        intersection.emplace_back(Triple(t[m].x + lambda * (t[mm].x-t[m].x), t[m].y + lambda * (t[mm].y-t[m].y), val));
                    }
                }
