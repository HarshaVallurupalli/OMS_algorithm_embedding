...
void SurfacePlot::Isolines2FloorC()
{
	...
	for (int k = 0; k != isolines(); ++k) 
	{
		...
		for (unsigned i=0; i!=actualDataC_->cells.size(); ++i)
		{
			...
			{
                nodes.emplace_back(actualDataC_->nodes[actualDataC_->cells[i][j]]);
			}
			...
			for (unsigned m = 0; m!=cellnodes; ++m)
			{
				...
				if ((val>=nodes[m].z && val<=nodes[mm].z) || (val>=nodes[mm].z && val<=nodes[m].z))
				{
					...
					{
                        intersection.emplace_back(nodes[m]);
                        intersection.emplace_back(nodes[mm]);
						continue;
					}
					
					lambda =  (val - nodes[m].z) / diff;
                    intersection.emplace_back(Triple(nodes[m].x + lambda * (nodes[mm].x-nodes[m].x), nodes[m].y + lambda * (nodes[mm].y-nodes[m].y), val));
				}
			}
