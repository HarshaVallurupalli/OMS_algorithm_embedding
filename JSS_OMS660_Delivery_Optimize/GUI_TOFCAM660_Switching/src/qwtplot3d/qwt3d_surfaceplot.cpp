...
void SurfacePlot::calculateHull()
{
    if (actualData_p->empty())
      ...
      setHull(ParallelEpiped(Triple(minX, minY, minZ), Triple(maxX, maxY, maxZ)));
}
    ...
    if(actualData_p->datatype == Qwt3D::POLYGON)
    {
        ...
        for(int i = 0; i <= actualDataG_->columns() - step; i += step){
            for(int j = 0; j <= actualDataG_->rows() - step; j += step){
                if(actualDataG_->vertices[i][j][2] <= minZ || actualDataG_->vertices[i][j][2] >= maxZ){
                    // hide dot
                }else{
                    ve->draw(Triple(actualDataG_->vertices[i][j][0], actualDataG_->vertices[i][j][1], actualDataG_->vertices[i][j][2]));
                }
            }
        }
    }
...
void SurfacePlot::setMinX(double val){
    minX = val;
}

void SurfacePlot::setMaxX(double val){
    maxX = val;
}

void SurfacePlot::setMinY(double val){
    minY = val;
}

void SurfacePlot::setMaxY(double val){
    maxY = val;
}

void SurfacePlot::setMinZ(double val){
    minZ = val;
}

void SurfacePlot::setMaxZ(double val){
    maxZ = val;
}

void SurfacePlot::setPointSize(double pointsize){
