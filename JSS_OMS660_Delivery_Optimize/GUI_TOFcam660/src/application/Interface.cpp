...
void Interface::requestDCS(const bool doStream, const QByteArray &userData){

    QByteArray outputData;

    uint16_t command = CommunicationConstants::CommandList::COMMAND_GET_DCS;
    uint8_t stream = boolToUint8(doStream);
...
void Interface::sendFirmwareUpdate(const QByteArray data)
{
    QByteArray outputData;

    uint16_t command = CommunicationConstants::CommandList::COMMAND_SEND_FIRMWARE_UPDATE;
