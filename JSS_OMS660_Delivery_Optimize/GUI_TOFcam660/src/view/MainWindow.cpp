    ...
    controller(context)
{
    ...
    connect(widgetData, SIGNAL(updateTimeScopeDCS(const char*, DataHeader&)),   &context.getMultiPixelStatistics(), SLOT(onDcsReceived(const char*, DataHeader&)));
    ...
    connect(this, SIGNAL(getFirmwareVersion()), widgetSaveLoad, SLOT(getFirmwareVersion()) );
    ...
    if(context.getSettings().getViewType() == Settings::ViewType::POINT_CLOUD_DISTANCE ||
        ...
        if(context.getSettings().log.isRecord()){
            ...
            if(context.getSettings().log.isDispalyRecord())
                widgetData->showData(pData, dataHeader);
...
{
    QDesktopServices::openUrl(QUrl("http://www.espros.com", QUrl::TolerantMode));
}
