...
{                
    ui->rbDCS0->setVisible(false);
    ui->rbDCS1->setVisible(false);
    ui->rbDCS2->setVisible(false);
    ui->rbDCS3->setVisible(false);
    ...
    case MeasurementType_e::MEASUREMENT_TYPE_DCS:
        ui->rbDCS0->setVisible(true);
        ui->rbDCS1->setVisible(true);
        ui->rbDCS2->setVisible(true);
        ui->rbDCS3->setVisible(true);
        this->setWindowTitle("PixelScope: DCS");
        seriesPixelScope->setName("DCS");
        ui->lMeasurementType->setText("Amplitude [LSB]:");
        break;        
    }
...
void WidgetPixelScope::onDCSDataReceived(const char *pData, DataHeader &dataHeader)
{
    ...
    if(dcsIndex == 1) idx = 2;
    else if(dcsIndex == 2) idx = 1;

    unsigned int index = dataHeader.width * dataHeader.height * idx;
    ...
    for(int y = 0; y < dataHeader.height; y++)
    {
        for(int x = 0; x < dataHeader.width; x++)
        {
            if(y == ySelection)
            {
                ...
                //Combint to a value
                int pixelDCS = (dcsMsb << 8) + dcsLsb;

                if(pixelDCS< 64000)
                    currentPoints.append(QPointF(x, pixelDCS));
            }
...
void WidgetPixelScope::handleAutoScale(QLineSeries *data[], unsigned int numSeries, QValueAxis *axis)
{
    ...
    if(max - min < MIN_DCS_DIFF_MAX_MIN){
        max = min + MIN_DCS_DIFF_MAX_MIN;
    }
...
void WidgetPixelScope::on_rbDCS0_toggled(bool checked)
{
    Q_UNUSED(checked)
    dcsIndex = 0;
}

void WidgetPixelScope::on_rbDCS1_toggled(bool checked)
{
    Q_UNUSED(checked)
    dcsIndex = 1;
}

void WidgetPixelScope::on_rbDCS2_toggled(bool checked)
{
    Q_UNUSED(checked)
    dcsIndex = 2;
}

void WidgetPixelScope::on_rbDCS3_toggled(bool checked)
{
    Q_UNUSED(checked)
    dcsIndex = 3;
}
