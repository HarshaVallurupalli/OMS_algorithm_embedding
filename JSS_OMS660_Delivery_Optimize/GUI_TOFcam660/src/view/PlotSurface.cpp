...
{    
    ...
    connect(&set.cloud, SIGNAL(sensorParameterChanged()), this, SLOT(sensorParameterChanged()));
...
void PlotSurface::updateGUI(int **distance, int **amplitude){
    ...
    double angle = set.cloud.getAngleObjectZ() * PI / 180;
    ...
    if(set.cloud.isCartesian()){
        ...
        uint x1 = static_cast<uint>(set.getRoiSettings().getX1());
        uint y1 = static_cast<uint>(set.getRoiSettings().getY1());
        uint x2 = static_cast<uint>(set.getRoiSettings().getX2());
        uint y2 = static_cast<uint>(set.getRoiSettings().getY2());

        lensTransform.runRoiTransform(distance, x1, y1, x2, y2, sin_angle, cos_angle, width/2, height/2);
        ...
        for(uint y=0; y<height; y++){
            for(uint x=0; x< width; x++){
                if(x < x1 || x > x2 || y < y1 || y > y2){
                    ...
                    tripData[x][y].x = lensTransform.xOut[x][y];
                    tripData[x][y].y = lensTransform.yOut[x][y];
                    tripData[x][y].z = lensTransform.zOut[x][y];
                }
            }
        }

        if(set.cloud.isPoints())
            loadFromData(tripData, width, height);
...
void PlotSurface::loadDefault(double angleX, double angleY, double angleZ, double zoom, double offsetX, double offsetY) {
    setRotation(angleX, angleY, angleZ);    
    setScale(1.0, 1.0, 1.0);
    setShift(0.0, 0.0, 0.0);
    setViewportShift(offsetX, offsetY);
    setZoom(zoom);
    setOrtho(false);
}
...
void PlotSurface::sensorParameterChanged()
{
    lensTransform.initialisation(0.02, 320, 240, set.cloud.getSensorCenterOffsetX(), set.cloud.getSensorCenterOffsetY(), set.cloud.isWideField());
}
