...
void Plot3D::mouseMoveEvent( QMouseEvent *e )
{
    ...
    Qt::KeyboardModifiers key_modifier = e->modifiers();

    if(zoomState_){

        key_modifier = Qt::AltModifier | Qt::ControlModifier;

    }else if(shiftState_){

        key_modifier = Qt::ControlModifier;

    }else if(rotationState_){

        key_modifier = Qt::ControlModifier | Qt::ShiftModifier;
    }

    MouseState bstate(e->buttons(), key_modifier);

	QPoint diff = e->pos() - lastMouseMovePosition_;

    setRotationMouse(bstate, 1, diff);
...
void Plot3D::wheelEvent( QWheelEvent *e )
{
    ...
    else setZoom(max(0.0,zoom() + step ));
}
        ...
        case QEvent::TouchEnd:
        {
            ...
            if (touchPoints.count() == 2) {
                ...
                if (touchEvent->touchPointStates() & Qt::TouchPointMoved) {
                    ...
                    double scaleFactor = zoom();
                    scaleFactor += ( currentScaleFactor * 0.1);
...
void Plot3D::setRotationKeyboard(KeyboardState kseq, double speed)
{
    ...
    qDebug()<< "+++++++++++++++++++++++Plot3D::setRotationKeyboard rot: " << new_xrot << new_yrot  << new_zrot; //TODO remove
