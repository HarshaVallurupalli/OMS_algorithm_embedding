...
public:
    ...
    void assignKeyboard(KeyboardState xrot_n, KeyboardState xrot_p, KeyboardState yrot_n, KeyboardState yrot_p,
                        KeyboardState zrot_n, KeyboardState zrot_p, KeyboardState xscale_n, KeyboardState xscale_p,
                        KeyboardState yscale_n, KeyboardState yscale_p, KeyboardState zscale_n, KeyboardState zscale_p,
                        KeyboardState zoom_n, KeyboardState zoom_p, KeyboardState xshift_n, KeyboardState xshift_p,KeyboardState yshift_n, KeyboardState yshift_p);
		
    bool keyboardEnabled() const; // Returns true, if the widget accept keyboard input from the user
    void setKeySpeed(double rot, double scale, double shift); // Sets speed for keyboard driven transformations
    void keySpeed(double& rot, double& scale, double& shift) const; // Gets speed for keyboard driven transformations
    ...
    double xLightRotation(unsigned idx = 0) const { return (idx<8) ? lights_[idx].rot.x : 0;} //Returns Light 'idx' rotation around X axis [-360..360] (some angles are equivalent)
    double yLightRotation(unsigned idx = 0) const { return (idx<8) ? lights_[idx].rot.y : 0;} // Returns Light 'idx' rotation around Y axis [-360..360] (some angles are equivalent)
    double zLightRotation(unsigned idx = 0) const { return (idx<8) ? lights_[idx].rot.z : 0;} // Returns Light 'idx' rotation around Z axis [-360..360] (some angles are equivalent)

    double xLightShift(unsigned idx = 0) const {return (idx<8) ? lights_[idx].shift.x : 0;} // Returns shift of Light 'idx 'along X axis (object coordinates)
    double yLightShift(unsigned idx = 0) const {return (idx<8) ? lights_[idx].shift.y : 0;} // Returns shift of Light 'idx 'along Y axis (object coordinates)
    double zLightShift(unsigned idx = 0) const {return (idx<8) ? lights_[idx].shift.z : 0;} // Returns shift of Light 'idx 'along Z axis (object coordinates)
    bool hasData() const { return (actualData_p) ? !actualData_p->empty() : false;} // Returns true if valid data available, false else

    
signals:		   
    void rotationChanged( double xAngle, double yAngle, double zAngle ); // Emitted, if the rotation is changed
    void shiftChanged( double xShift, double yShift, double zShift ); // Emitted, if the shift is changed
    void vieportShiftChanged( double xShift, double yShift ); // Emitted, if the viewport shift is changed
    void scaleChanged( double xScale, double yScale, double zScale ); // Emitted, if the scaling is changed
    void zoomChanged(double); // Emitted, if the zoom is changed
    void projectionChanged(bool); // Emitted, if the projection mode is changed    

public slots:

    void setRotation(double xVal, double yVal, double zVal, bool update = true);
    void setShift(double xVal, double yVal, double zVal, bool update = true);
    void setViewportShift(double xVal, double yVal, bool update = true);
    void setScale(double xVal, double yVal, double zVal, bool update = true);
    void setZoom(double val , bool update = true);
    void setOrtho(bool);
    ...
    virtual bool saveVector(QString const& fileName, QString const& format, VectorWriter::TEXTMODE text, VectorWriter::SORTMODE sortmode); //Saves content to vector format
...
protected:
    ...
    enum OBJECTS { DataObject, LegendObject, NormalObject, DisplayListSize  }; // only to have a vector length ...
    std::vector<GLuint> displaylists_p;
    Qwt3D::Data* actualData_p;


private:
    struct Light  {  Light() : unlit(true){} bool unlit; Qwt3D::Triple rot; Qwt3D::Triple shift; };
    ...
    MouseState xrot_mstate_, yrot_mstate_, zrot_mstate_, xscale_mstate_, yscale_mstate_, zscale_mstate_, zoom_mstate_, xshift_mstate_, yshift_mstate_;
    ...
    // keyboard
    bool kpressed_;
    KeyboardState xrot_kstate_[2], yrot_kstate_[2],  zrot_kstate_[2], xscale_kstate_[2], yscale_kstate_[2], zscale_kstate_[2], zoom_kstate_[2], xshift_kstate_[2], yshift_kstate_[2];
    ...
    bool zoomState_;
    bool rotationState_; //lsi
    bool shiftState_; //lsi
    double totalScaleFactor; //lsi
};
