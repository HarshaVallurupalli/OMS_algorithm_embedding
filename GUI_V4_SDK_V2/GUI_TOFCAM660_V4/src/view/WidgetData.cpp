...
void WidgetData::updateImage(ApplicationContext &context,QLabel &label, const QImage &image)
{
...
void WidgetData::showDistance(ApplicationContext &context,const char *pData, DataHeader &dataHeader)
{
    int y0 = 0;
    ...
    QImage imageEdges(dataHeader.width, dataHeader.height - 2*y0, QImage::Format_RGB888);
    ...
    for(int y = 0; y < dataHeader.height; y++){
        for(int x = 0; x < dataHeader.width; x++, index++){
            ...
            if(y >= y0 && y < height){
                ...
                unsigned int pixelEdges;
                if( context.getSettings().filter.isEdgeFilter())
                {
                   if(pixelDistance==64008)
                        {
                       pixelEdges= pixelDistance;
                        }
                       else
                        {
                       pixelEdges=0;
                ...
                imageEdges.setPixelColor(x, y-y0, imageColorizerDistance.getColor(pixelEdges, ImageColorizer::REDTOBLUE));
            }
        }
    }
if( context.getSettings().filter.isEdgeFilter())
...
ui->labelRightImage->setImage(imageEdges);
...
void WidgetData::showData(ApplicationContext &context,const char *pData, DataHeader &dataHeader)
{
    ...
    case TOFCAM_DATA_DISTANCE_AMPLITUDE:
        ...
        break;
    case TOFCAM_DATA_DISTANCE:
        showDistance(context,pData, dataHeader);
        break;
    case TOFCAM_DATA_GRAYSCALE:
