...
Controller::Controller(ApplicationContext &context):context(context)
{
  ...
  connect(&context.getSettings().algorithm, static_cast<void (SettingsAlgorithm::*)()>(&SettingsAlgorithm::algorithmChanged), this, &Controller::onAlgorithmChanged);
...
void  Controller::onAlgorithmChanged(){

    bool skeletonDetectionAlgorithm = context.getSettings().algorithm.isSDAEnabled();
    uint16_t movementdetectionWindowSize = static_cast<uint16_t>(context.getSettings().algorithm.getMDAWindowSize());
    uint16_t movementdetectionImageType = static_cast<uint16_t>(context.getSettings().algorithm.getMDAImageType());

    context.getInterface().setAlgorithm(skeletonDetectionAlgorithm, movementdetectionWindowSize, movementdetectionImageType);
...
void Controller::requestMovementDetection(const bool isStream){
    if (isStream)
    {
        context.setApplicationState(TypeApplicationState::STATE_STREAM);
    }
    ...
    QByteArray userData = context.getSettings().getUserData();
    context.getInterface().requestMovementDetection(isStream, userData);
}
